...
### mcporter (any MCP server)

Whenever you invoke **any** MCP server through **mcporter**, you **must** use explicit **`--server <alias>`** and **`--tool '<full tool name>'`** after `call`. Do **not** rely on positional arguments such as `call <serverAlias> "<toolName>"` with no flags.

**Required shape** (adjust `--config`, server alias, tool name, and key=value args to match the skill or project):
