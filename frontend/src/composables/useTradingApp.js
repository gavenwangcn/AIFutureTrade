...
let portfolioRefreshInterval = null // 投资组合数据自动刷新定时器（轮询方式，默认5秒，包含账户总值、可用现金、已实现盈亏、未实现盈亏、每日收益率）
  ...
  /**
   * 执行盯盘（可选）：后端创建容器 aifuturetrade-model-look-1；日常由 docker compose 启动时可不用
   */
  const handleExecuteMarketLook = async () => {
    ...
    try {
      const result = await modelApi.executeMarketLook()
  ...
  /**
   * 关闭盯盘（可选）：删除容器 aifuturetrade-model-look-1
   */
  const handleStopMarketLook = async () => {
    ...
    try {
      const result = await modelApi.stopMarketLook()
