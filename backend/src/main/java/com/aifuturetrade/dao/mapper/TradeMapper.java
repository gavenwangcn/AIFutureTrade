...
public interface TradeMapper extends BaseMapper<TradeDO> {
     ...
     * 按策略名称分组统计：交易次数、胜率、平均盈利、平均亏损、盈亏比、期望值
     * 关联逻辑：通过model_id、symbol、signal和时间窗口（5分钟内）来匹配策略决策和交易记录
     * @param modelId 模型ID（UUID格式）
     * @return 策略统计信息列表，每个元素包含：strategy_name, trade_count, win_rate, avg_profit, avg_loss, profit_loss_ratio, expected_value
     */
    @Select("SELECT " +
            ...
            "    CASE " +
            "        WHEN COUNT(DISTINCT t.id) = 0 THEN NULL " +
            "        WHEN (CAST(SUM(CASE WHEN t.pnl > 0 THEN 1 ELSE 0 END) AS DECIMAL(10, 5)) / COUNT(DISTINCT t.id)) IS NULL THEN NULL " +
            "        WHEN AVG(CASE WHEN t.pnl > 0 THEN t.pnl ELSE NULL END) IS NULL THEN NULL " +
            "        WHEN AVG(CASE WHEN t.pnl < 0 THEN t.pnl ELSE NULL END) IS NULL THEN NULL " +
            "        ELSE (CAST(SUM(CASE WHEN t.pnl > 0 THEN 1 ELSE 0 END) AS DECIMAL(10, 5)) / COUNT(DISTINCT t.id)) * " +
            "             AVG(CASE WHEN t.pnl > 0 THEN t.pnl ELSE NULL END) - " +
            "             (1 - CAST(SUM(CASE WHEN t.pnl > 0 THEN 1 ELSE 0 END) AS DECIMAL(10, 5)) / COUNT(DISTINCT t.id)) * " +
            "             ABS(AVG(CASE WHEN t.pnl < 0 THEN t.pnl ELSE NULL END)) " +
