...
用法:
    python trade/tests/run_tests.py [test_name]
    或
    python -m trade.tests.run_tests [test_name]

示例:
    python trade/tests/run_tests.py test_database_clickhouse
    python trade/tests/run_tests.py  # 运行所有测试
"""
import sys
import importlib
...
def run_test(test_name: str = None):
    ...
    if test_name:
        ...
        print("=" * 80)
        try:
            test_module = importlib.import_module(f"trade.tests.{test_name}")
        ...
        for test_file in test_files:
            ...
            print("=" * 80)
            try:
                test_module = importlib.import_module(f"trade.tests.{test_file}")
                if hasattr(test_module, 'main'):
                    result = test_module.main()
