...
class StrategyTrader(Trader):
    ...
    def make_look_decision(
        ...
        """
        执行单条盯盘策略（不依赖 model_strategy 列表，由调用方传入 strategys 行）。
        """
        ...
        try:
            ...
            if not decision_result or not isinstance(decision_result, dict):
                return {'decisions': {}, 'prompt': None, 'raw_response': None, 'cot_trace': strategy_name, 'skipped': False}
            decisions = decision_result.get('decisions') or {}
            return {
            ...
            return {'decisions': {}, 'prompt': None, 'raw_response': str(e), 'cot_trace': strategy_name, 'skipped': False}
