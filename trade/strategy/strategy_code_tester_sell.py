...
class StrategyCodeTesterSell:
    ...
    def test_strategy_code(
        ...
        if passed:
            ...
            logger.error(f"[StrategyCodeTesterSell] 测试失败，提交到测试模块的AI生成策略代码（完整代码，未截取）:")
            logger.error(f"[StrategyCodeTesterSell] 代码统计: 总长度={code_length} 字符, 总行数={code_lines} 行")
            logger.error(f"[StrategyCodeTesterSell] --- 策略代码开始（完整代码） ---")
            ...
            if code_length <= chunk_size:
                ...
                for idx, chunk in enumerate(chunks, 1):
                    logger.error(f"[StrategyCodeTesterSell] [代码片段 {idx}/{len(chunks)}] (字符 {idx*chunk_size-chunk_size+1}-{min(idx*chunk_size, code_length)}):\n{chunk}")
            logger.error(f"[StrategyCodeTesterSell] --- 策略代码结束（完整代码） ---")
    ...
    def _test_execution(self, strategy_code: str, strategy_name: str) -> Dict:
        ...
        try:
            ...
            if sell_result is None:
                ...
                decisions = sell_result.get('decisions', {})
                if not isinstance(decisions, dict):
                    ...
                    for sym, val in decisions.items():
                        if not isinstance(val, list):
                            errors.append(
                                ...
                                f"正确示例: {{\"BTCUSDT\": [{{\"signal\": \"close_position\", \"quantity\": 100, ...}}]}}"
                            )
                            break
                    ...
                    if not errors:
                        ...
                        logger.debug(f"[StrategyCodeTesterSell] ✓ 卖出决策执行成功，格式 Dict[symbol, List[decision]] 正确，共 {len(decisions)} 个 symbol、{total_count} 条决策")
        except Exception as e:
