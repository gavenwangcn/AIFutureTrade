...
def trading_sell_loop_for_model(model_id_int: int, engine: TradingEngine, db):
    """单个模型的卖出交易循环"""
    logger.info(f"Trading sell loop started for model {model_id_int}")
    ...
    while auto_trading:
        try:
            ...
            # 检查模型的 auto_sell_enabled 字段（使用整数ID）
            if not models_db.is_model_auto_sell_enabled(model_id_int):
                logger.info(f"SKIP: Model {model_id_int} - auto_sell_enabled=0, skipping AI sell decision")
            else:
                ...
                if model:
                    ...
                    if trade_type == 'strategy':
                        ...
                        if not sell_strategies:
                            logger.info(f"SKIP: Model {model_id_int} - trade_type=strategy but no sell strategy configured, skipping sell decision")
                        else:
                            # 执行卖出决策
                            logger.info(f"\nEXEC SELL: Model {model_id_int} - auto_sell_enabled=1, executing AI sell decision")
                            result = engine.execute_sell_cycle()
                            
                            if result.get('success'):
                                logger.info(f"OK: Model {model_id_int} sell cycle completed")
                                ...
                                logger.warning(f"Model {model_id_int} sell cycle failed: {error}")
                    else:
                        # AI交易类型，直接执行
                        logger.info(f"\nEXEC SELL: Model {model_id_int} - auto_sell_enabled=1, executing AI sell decision")
                        result = engine.execute_sell_cycle()
    ...
    logger.info(f"Trading sell loop stopped for model {model_id_int}")
...
def main():
    ...
    # 启动卖出循环（传递整数ID，避免循环中重复转换）
    try:
        trading_sell_loop_for_model(model_id_int, engine, db)
    except KeyboardInterrupt:
